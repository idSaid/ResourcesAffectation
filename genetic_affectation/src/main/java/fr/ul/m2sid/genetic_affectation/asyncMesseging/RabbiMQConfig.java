...
public class RabbiMQConfig implements RabbitListenerConfigurer {
    ...
    public final static String P1_ALLOCATION_QUEUE = "AllocationQueue";
    ...
    Queue allocationQueue() {
        return new Queue(P1_ALLOCATION_QUEUE, false);
    }
    ...
    Binding affectation_binding(Queue affectationQueue, TopicExchange exchange) {
        return BindingBuilder.bind(affectationQueue).to(exchange).with(P1_AFFECTATION_QUEUE);
    }

    @Bean
    Binding allocation_binding(Queue allocationQueue, TopicExchange exchange) {
        return BindingBuilder.bind(allocationQueue).to(exchange).with(P1_ALLOCATION_QUEUE);
    }
